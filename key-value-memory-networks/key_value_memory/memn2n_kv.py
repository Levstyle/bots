...
class MemN2N_KV(object):
    ...
    def __init__(self, batch_size, vocab_size,
        ...
        :param vocab_size: The size of the vocabulary (should include the nil word). The nil word one-hot encoding should be 0
        :param query_size: largest number of words in question
        :param story_size: largest number of words in story
        :param memory_key_size: the size of memory slots for keys
        :param memory_value_size: the size of memory slots for values
        ...
        :param reader:
        :param l2_lambda:
        :param name: Name of the End-To-End Memory Network
        """
        ...
        self.A = tf.get_variable(
            'A',
            shape=[self._feature_size, self.reader_feature_size],
            initializer=tf.contrib.layers.xavier_initializer()
        )
        self.TK = tf.get_variable(
            'TK',
            shape=[self._memory_value_size, self.reader_feature_size],
            initializer=tf.contrib.layers.xavier_initializer()
        )
        self.TV = tf.get_variable(
            'TV',
            shape=[self._memory_value_size, self.reader_feature_size],
            initializer=tf.contrib.layers.xavier_initializer()
        )
