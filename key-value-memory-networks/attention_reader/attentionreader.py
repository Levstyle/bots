...
class AttentionReader(object):
    ...
    def __init__(self, batch_size, vocab_size, embedding_size, doc_num,
        ...
        with tf.device('/cpu:0'), tf.name_scope("embedding"):
            ...
            self.W = tf.concat(
                [nil_word_slot, tf.Variable(tf.random_uniform([vocab_size - 1, embedding_size], -1.0, 1.0))],
                0
            )
        ...
        for i in range(self._k):
            with tf.name_scope(
                ...
                # get gru cell output
                q_outputs, q_fw, q_bw = tf.contrib.rnn.static_bidirectional_rnn(
                    rnn_fw, rnn_bw, q, dtype=tf.float32)
                q_r = tf.concat([q_fw, q_bw], 1)
            ...
            if i == 0:
                with tf.variable_scope('doc_gru'):
                    doc_outputs, _, __ = tf.contrib.rnn.static_bidirectional_rnn(
        ...
        doc_r = tf.reduce_sum(tf.multiply(t_doc_outputs, probs_e), 1)
